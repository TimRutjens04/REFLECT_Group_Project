...
class PipelineDepth:
    def __init__(self, args: argparse.Namespace, provider: Any = None) -> None:
        self.args = args
        ...
        # When a provider (e.g. VideoRgbdFrameProvider) is supplied, RGB/depth
        # frames are read through it instead of opening raw zarr arrays here.
    ...
    def load_tracking_jsonl(self) -> None:
        ...
        if self.args.raw_episode_dir:
            self.raw_episode_dir = Path(self.args.raw_episode_dir).expanduser().resolve()
        elif self.provider is not None and getattr(self.provider, "task_root", None) is not None:
            self.raw_episode_dir = Path(self.provider.task_root).expanduser().resolve()
        else:
            self.raw_episode_dir = self.workspace_root / "Data" / "Data" / "Real Data" / self.sequence_id
        self.output_dir = (
    ...
    def open_raw_rgbd(self) -> None:
        if self.provider is not None:
            self.frame_count = int(self.provider.n_frames)
            print("RGB-D source: data loader provider", type(self.provider).__name__)
            ...
            self.frame_count = int(self.depth_zarr.shape[0])
    ...
    def raw_index(self, timestamp: float) -> int:
        ...
        return int(np.clip(index, 0, max(0, self.frame_count - 1)))

    def get_rgb_by_timestamp(self, timestamp: float) -> np.ndarray:
        if self.provider is not None:
            return np.asarray(self.provider.get_frame(self.raw_index(timestamp)).rgb)
        return np.asarray(self.color_zarr[self.raw_index(timestamp)])

    def get_depth_by_timestamp(self, timestamp: float) -> np.ndarray:
        if self.provider is not None:
            return np.asarray(self.provider.get_frame(self.raw_index(timestamp)).depth)
...
def run_depth_scene_graph(
    tracking_jsonl: str | Path,
    provider: Any = None,
    output_dir: str | Path | None = None,
    extra_args: list[str] | None = None,
) -> "PipelineDepth":
    """Run the depth + scene graph stage in-process.

    Pass the same data loader provider used upstream (e.g.
    ``VideoRgbdFrameProvider``) so RGB/depth frames are read through it instead
    of opening raw zarr arrays here, and point ``tracking_jsonl`` at the
    tracker/validator handoff file (e.g. ``real_world/jsonl/validation.jsonl``).
    """
    argv = ["--tracking-jsonl", str(tracking_jsonl)]
    if output_dir is not None:
        argv += ["--output-dir", str(output_dir)]
    if extra_args:
        argv += extra_args
    pipeline = PipelineDepth(parse_args(argv), provider=provider)
    pipeline.run()
    return pipeline
