...
# wording to phrasing the grounding model recognizes more reliably.
...
# LocateAnything prompt templates (verbatim from the model card).
# Detection / multi-category uses one prompt with all classes; per-object
# fallback uses single-instance grounding to mirror Grounding DINO's
# "best box per label" behaviour.
_MULTI_TEMPLATE = (
    "Locate all the instances that matches the following description: {desc}."
)
_SINGLE_TEMPLATE = (
    "Locate a single instance that matches the following description: {desc}."
)
# Categories may be joined with "</c>" per the model card.
_CATEGORY_SEP = "</c>"


@dataclass
class PromptPlan:
    primary: str  # multi-object prompt, e.g. "Locate all ...: red cup</c>blue plate."
    singles: list[str]  # per-object single-instance fallback prompts
    objects: list[str]  # canonical labels in the order they appear in primary


class PromptStrategy:
    """Builds LocateAnything-3B prompts from a RoboFail task description.
    ...
    def from_task(self, task: Task) -> PromptPlan:
        ...
        if not objects:
            return PromptPlan(
                primary=self._format_multi(["object"]),
                singles=[self._format_single("object")],
                objects=["object"],
            )
    ...
    def _format_multi(objects: list[str]) -> str:
        return _MULTI_TEMPLATE.format(desc=_CATEGORY_SEP.join(objects))

    @staticmethod
    def _format_single(obj: str) -> str:
        return _SINGLE_TEMPLATE.format(desc=obj)
