...
try:
    from constants import real_world_name_map
except ImportError:
    # Avoid importing reflect-main/main/constants.py when it shadows local constants.
    _constants_path = Path(__file__).resolve().parent / "constants.py"
    _spec = importlib.util.spec_from_file_location(
        "real_world_constants", _constants_path
    )
    _module = importlib.util.module_from_spec(_spec)
    assert _spec is not None and _spec.loader is not None
    _spec.loader.exec_module(_module)
    real_world_name_map = _module.real_world_name_map

# RealSense intrinsic matrix
intrinsics_matrix = np.array(
    [[914.27246, 0.0, 647.0733], [0.0, 913.2658, 356.32526], [0.0, 0.0, 1.0]]
)
...
def visualize_box(total_points, total_colors, box, color):
    for i in np.arange(box[0][0], box[4][0], 100):
        total_points = torch.cat(
            (total_points, torch.tensor([[i, box[0][1], box[0][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[i, box[0][1], box[4][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[i, box[4][1], box[0][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[i, box[4][1], box[4][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

    for i in np.arange(box[0][1], box[4][1], 100):
        total_points = torch.cat(
            (total_points, torch.tensor([[box[0][0], i, box[0][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[box[0][0], i, box[4][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[box[4][0], i, box[0][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[box[4][0], i, box[4][2]]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

    for i in np.arange(box[0][2], box[4][2], 100):
        total_points = torch.cat(
            (total_points, torch.tensor([[box[0][0], box[0][1], i]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[box[0][0], box[4][1], i]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[box[4][0], box[0][1], i]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)

        total_points = torch.cat(
            (total_points, torch.tensor([[box[4][0], box[4][1], i]])), 0
        )
        total_colors = torch.cat((total_colors, torch.tensor([color])), 0)
...
def is_duplicate_bbox(idx, outputs):
    for idx2 in range(idx + 1, len(outputs["labels"])):
        ...
        print(
            f"iou between {outputs['labels'][idx]} and {outputs['labels'][idx2]}: ",
            temp_iou,
        )
        if (
            idx2 != idx
            and outputs["labels"][idx2] == outputs["labels"][idx]
            and bb_iou(outputs["bbox_2d"][idx], outputs["bbox_2d"][idx2]) > 0.25
        ):
            return True
    return False
...
def confirm_obj_det(args, rgb, outputs, object_list, step_idx):
    ...
    outputs["scores"], outputs["labels"], outputs["pred_masks"], outputs["bbox_2d"] = (
        zip(
            *sorted(
                zip(
                    outputs["scores"],
    ...
    for idx in range(len(outputs["bbox_2d"])):
        ...
        sorted_obj_names, sorted_scores = get_nn_text(
            object_list, obj_name_feats, img_feats
        )
        ...
        for i in range(len(sorted_scores)):
            if i == 0:
                ...
                sorted_scores[i] > 0.23 and label == sorted_obj_names[i]
            ):  # change from 0.25 to 0.23 for sauteeCarrot4
                clip_conf = True
                break
        if clip_conf and not is_duplicate_bbox(idx, outputs):
            if len(new_outputs["pred_masks"]) == 0:
                new_outputs["pred_masks"] = np.expand_dims(
                    outputs["pred_masks"][idx], axis=0
                )
                new_outputs["bbox_2d"] = np.expand_dims(outputs["bbox_2d"][idx], axis=0)
            else:
                new_outputs["pred_masks"] = np.concatenate(
                    (
                        new_outputs["pred_masks"],
                        np.expand_dims(outputs["pred_masks"][idx], axis=0),
                    )
                )
                new_outputs["bbox_2d"] = np.concatenate(
                    (
                        new_outputs["bbox_2d"],
                        np.expand_dims(outputs["bbox_2d"][idx], axis=0),
                    )
                )
            new_outputs["scores"] = np.append(
                new_outputs["scores"], outputs["scores"][idx]
            )
            new_outputs["labels"] = np.append(
                new_outputs["labels"], outputs["labels"][idx]
            )
        # plt.imshow(cropped_img)
        # plt.show()
    ...
    clip_processed_im = plot_results(
        rgb,
        new_outputs["scores"],
        new_outputs["bbox_2d"],
        new_outputs["labels"],
    ...
    plt.axis("off")
    plt.savefig(
        f"real_world/state_summary/{args.folder_name}/mdetr_obj_det/clip_processed_det/{step_idx}.png"
    )
    plt.close()
    return new_outputs
...
def obj_det(rgb, object_list, detector, step_idx, args):
    ...
    plt.axis("off")
    plt.savefig(
        f"real_world/state_summary/{args.folder_name}/mdetr_obj_det/images/{step_idx}.png"
    )
    plt.close()
    ...
    if args.obj_det == "detic":
        ...
        for idx, single_obj_prompt in enumerate(object_list):
            ...
            if len(outputs["pred_masks"]) == 0:
                ...
                outputs["pred_masks"] = np.concatenate(
                    (outputs["pred_masks"], retval["masks"])
                )
                outputs["bbox_2d"] = np.concatenate(
                    (outputs["bbox_2d"], retval["bbox_2d"])
                )
        ...
        plt.axis("off")
        plt.savefig(
            f"real_world/state_summary/{args.folder_name}/mdetr_obj_det/det/{step_idx}.png"
        )
        plt.close()
...
def get_scene_graph(
    ...
    if detector is not None:
        ...
        with open(
            f"real_world/state_summary/{args.folder_name}/{args.obj_det}_obj_det/det/{step_idx}.pickle",
            "wb",
        ) as f:
        ...
        if args.obj_det == "detic":
            ...
            with open(
                f"real_world/state_summary/{args.folder_name}/{args.obj_det}_obj_det/clip_processed_det/{step_idx}.pickle",
                "rb",
            ) as f:
                outputs = pickle.load(f)
    ...
    for idx in range(outputs["total_detections"]):
        ...
        _, ind = voxel_down_pcd.remove_statistical_outlier(
            nb_neighbors=1500, std_ratio=0.1
        )
        inlier = voxel_down_pcd.select_by_index(ind)

        pcd_dict[label] = np.array(inlier.points)
        if (
            label in ["fridge", "coffee machine", "table"]
            and label in total_points_dict
        ):
            total_points_dict[label] = np.concatenate(
    ...
    for i, label in enumerate(total_points_dict.keys()):
        if total_points is None:
            ...
            total_points = torch.cat(
                (total_points, torch.tensor(total_points_dict[label])), 0
            )
            c = torch.tensor(COLORS[i % len(COLORS)])
            total_colors = torch.cat(
                (total_colors, c.repeat(len(total_points_dict[label]), 1)), 0
            )
    ...
    if total_points is not None:
        ...
        o3d.io.write_point_cloud(
            "real_world/scene/{}/scene_{}.ply".format(args.folder_name, step_idx),
            saved_pcd,
        )
    ...
    obj_held = local_sg.add_agent(
        meta_data["data/gripper_pos"][step_idx], step_idx, args
    )
