...
def plot_inference_segmentation(im, caption, seg_model):
    ...
    # Keep high-confidence predictions; threshold is configurable for dataset tuning.
    probas = 1 - outputs["pred_logits"].softmax(-1)[0, :, -1].cpu()
    det_thresh = float(os.getenv("MDETR_DET_THRESH", "0.94"))
    keep = (probas > det_thresh).cpu()
    ...
    if len(masks) != 0:
        for mask in masks:
            ...
            eroded_mask = cv2.erode(
                np.array(mask, dtype=np.float32), kernel, iterations=2
            )
    ...
    # Newer Transformers versions removed batch_encode_plus on some fast tokenizers.
    ...
    labels = [predicted_spans.get(k, caption).strip() or caption for k in range(n_kept)]
