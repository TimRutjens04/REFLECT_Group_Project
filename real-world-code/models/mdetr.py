...
from .mdetr_utils import dist
from .mdetr_utils import box_ops
from .mdetr_utils.metrics import accuracy
from .mdetr_utils.misc import NestedTensor, interpolate
    ...
    def __init__(
        ...
        if qa_dataset is not None:
            if split_qa_heads:
                ...
                # TODO: make this more general
                assert qa_dataset == "gqa", (
                    "Clevr QA is not supported with unified head"
                )
                self.answer_head = nn.Linear(hidden_dim, 1853)

    def forward(
        self, samples: NestedTensor, captions, encode_and_save=True, memory_cache=None
    ):
...
class QACriterionClevr(nn.Module):
    ...
    def forward(self, output, answers):
        ...
        loss["accuracy_answer_binary"] = (
            bin_acc[is_binary].sum() / is_binary.sum()
            if is_binary.any()
...
def build(args):
    ...
    if args.do_qa:
        assert not (
            (
                "clevr" in args.combine_datasets
                or "clevr_question" in args.combine_datasets
            )
