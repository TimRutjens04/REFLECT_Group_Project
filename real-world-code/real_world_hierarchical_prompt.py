...
try:
    ...
    # AudioCLIP is optional; allow non-audio runs and lightweight imports.
    def get_sound_events(*args, **kwargs):
        return {}
    ...
    for edge in scene_graph.edges.values():
        ...
        if edge.edge_type == "near":
            if (end_node_name, start_node_name) in scene_graph.edges:
                other_edge = scene_graph.edges[(end_node_name, start_node_name)]
                if (
                    other_edge.edge_type == "on top of"
                    or other_edge.edge_type == "inside"
                    or other_edge.edge_type == "near"
                ):
                    continue
...
def save_L1_images(args, task_info):
    ...
    key_frames = []
    with open(
        "real_world/state_summary/{}/L1_key_frames.txt".format(args.folder_name), "r"
    ) as f:
        frames = f.readlines()
    ...
    for step_idx in key_frames:
        rgb = imread(
            f"real_world/data/{args.folder_name}/videos/0/0/color/{step_idx}.0.0.0"
        )
        depth = imread(
            f"real_world/data/{args.folder_name}/videos/0/0/depth/{step_idx}.0.0"
        )
...
def run_wo_sound(args, task_info):
    ...
    if not os.path.exists(
        f"real_world/state_summary/{args.folder_name}/state_summary_L2_wo_sound.txt"
    ):
        with open(
            "real_world/state_summary/{}/{}".format(
                args.folder_name, "state_summary_L2.txt"
            ),
            "r",
        ) as f:
        ...
        for caption in L2_captions:
            if "Auditory observation" in caption:
                L2_captions_wo_sound.append(
                    caption[: caption.find("Auditory observation") - 1]
                )
            else:
    ...
    if not os.path.exists(
        f"real_world/state_summary/{args.folder_name}/state_summary_L1_wo_sound.txt"
    ):
        with open(
            "real_world/state_summary/{}/{}".format(
                args.folder_name, "state_summary_L1.txt"
            ),
            "r",
        ) as f:
...
def run_BLIP2(args, task):
    ...
    if not os.path.exists(
        f"real_world/state_summary/{args.folder_name}/{reasoning_json_name}"
    ):
        reasoning_dict = {}
        ...
        reason_prompt["user"] = prompt_info["prompt-simple-qa-2"][
            "template-user"
        ].replace("[TASK_NAME]", task["name"])
        reason_prompt["user"] = reason_prompt["user"].replace(
        ...
        state_summary_L1 = ""
        with open(
            "real_world/state_summary/{}/state_summary_L1_BLIP2.txt".format(
                args.folder_name
            ),
            "r",
        ) as f:
        ...
        reason_prompt["user"] = reason_prompt["user"].replace(
            "[L1_SUMMARY]", state_summary_L1
        )
        reason_prompt["user"] = reason_prompt["user"].replace(
            "[L2_SUMMARY]", get_robot_plan(args, step=None, with_obs=False)
        )
        print(reason_prompt["user"])

        reason, _ = llm_prompter.query(
            prompt=reason_prompt,
            sampling_params=prompt_info["prompt-simple-qa-2"]["params"],
            save=prompt_info["prompt-simple-qa-2"]["save"],
            save_dir=f"LLM/{f_name}",
        )

        # get failure steps
        step_prompt = {}
        step_prompt["system"] = prompt_info["prompt-simple-qa-step"]["template-system"]
        step_prompt["user"] = prompt_info["prompt-simple-qa-step"][
            "template-user"
        ].replace("[PREV_PROMPT]", reason_prompt["user"] + " " + reason)
        # print(step_prompt['user'])
        step, _ = llm_prompter.query(
            prompt=step_prompt,
            sampling_params=prompt_info["prompt-simple-qa-step"]["params"],
            save=prompt_info["prompt-simple-qa-step"]["save"],
            save_dir=f"LLM/{f_name}",
        )
        ...
        with open(
            "real_world/state_summary/{}/{}".format(
                args.folder_name, reasoning_json_name
            ),
            "w",
        ) as f:
...
def LLM_direct_summary(args, task_info):
    ...
    pickle_names = os.listdir(
        f"real_world/state_summary/{args.folder_name}/local_graphs"
    )
    steps = sorted([int(p.split(".")[0].split("_")[-1]) for p in pickle_names])
    state_summary_L0 = ""
    with open(
        "real_world/state_summary/{}/state_summary_L1.txt".format(args.folder_name), "r"
    ) as f:
    ...
    if not os.path.exists(f"real_world/state_summary/{args.folder_name}/summary.txt"):
        ...
        prompt["user"] = prompt_info["prompt-direct-summary"]["template-user"].replace(
            "[TASK_NAME]", task["name"]
        )
        prompt["user"] = prompt["user"].replace(
            "[WORLD_STATE_HISTORY]", state_summary_L0
        )

        summary, _ = llm_prompter.query(
            prompt=prompt,
            sampling_params=prompt_info["prompt-direct-summary"]["params"],
            save=prompt_info["prompt-direct-summary"]["save"],
            save_dir=f"LLM/{f_name}",
        )
        print(summary)
    ...
    if not os.path.exists(
        ...
        reason_prompt["user"] = prompt_info["prompt-simple-qa"][
            "template-user"
        ].replace("[TASK_NAME]", task["name"])
        ...
        reason, _ = llm_prompter.query(
            prompt=reason_prompt,
            sampling_params=prompt_info["prompt-simple-qa"]["params"],
            save=prompt_info["prompt-simple-qa"]["save"],
            save_dir=f"LLM/{f_name}",
        )
...
def LLM_direct_reasoning(args, task_info):
    ...
    if not os.path.exists(
        ...
        state_summary_L1 = ""
        with open(
            "real_world/state_summary/{}/state_summary_L1.txt".format(args.folder_name),
            "r",
        ) as f:
            L1_captions = f.readlines()
        state_summary_L1 = "".join(L1_captions)
...
def run_reasoning(args, task, global_sg):
    ...
    if os.path.exists(
        ...
        # if False:
        with open(
            f"real_world/state_summary/{args.folder_name}/{reasoning_json_name}", "r"
        ) as f:
    ...
    with open(
        "real_world/state_summary/{}/{}".format(args.folder_name, summary_file_name),
        "r",
    ) as f:
        L2_captions = f.readlines()
    ...
    for caption in L2_captions:
        ...
        prompt["user"] = (
            prompt_info["prompt-action-binary"]["template-user"]
        ...
        ans, _ = llm_prompter.query(
            prompt=prompt,
            sampling_params=prompt_info["prompt-action-binary"]["params"],
            save=prompt_info["prompt-action-binary"]["save"],
            save_dir=save_dir,
        )
    ...
    if len(selected_caption) != 0:
        ...
        for _, caption in enumerate(explain_captions):
            if step_name in caption:
                ...
                prompt["user"] = prompt_info[prompt_name]["template-user"].replace(
                    "[ACTION]", action
                )
                ...
                if args.ablation_type == 3:
                    prompt["user"] = prompt["user"].replace(
                        "[OBSERVATION]", caption[caption.find("Goal") :]
                    )
                else:
                    prompt["user"] = prompt["user"].replace(
                        "[OBSERVATION]", caption[caption.find("Action") :]
                    )
                ans, log_prob = llm_prompter.query(
                    prompt=prompt,
                ...
                if args.ablation_type == 3:
                    ...
                    prompt = {}
                    prompt["system"] = prompt_info["prompt-reason-step"][
                        "template-system"
                    ]
                    prompt["user"] = prompt_info["prompt-reason-step"][
                        "template-user"
                    ].replace("[FAILURE_REASON]", ans)
                    time_steps, log_prob = llm_prompter.query(
                        prompt=prompt,
                        sampling_params=prompt_info["prompt-reason-step"]["params"],
                        save=prompt_info["prompt-reason-step"]["save"],
                        save_dir=save_dir,
                    )
                    print(
                        "[LLM RESPONSE] time steps:", time_steps, time_steps.split(", ")
                    )
                    reasoning_dict["pred_failure_step"] = [
                        time_step.replace(",", "")
                        for time_step in time_steps.split(", ")
                    ]
                break
    else:
        print(">>> All actions are executed successfully, run plan-level analysis...")
        prompt["system"] = prompt_info["prompt-plan"]["template-system"]
        prompt["user"] = prompt_info["prompt-plan"]["template-user"].replace(
            "[TASK_NAME]", task["name"]
        )
        prompt["user"] = prompt["user"].replace(
            "[SUCCESS_CONDITION]", task["success_condition"]
        ...
        prompt["user"] = prompt["user"].replace(
            "[CURRENT_STATE]", get_scene_text(global_sg)
        )
        prompt["user"] = prompt["user"].replace(
            "[OBSERVATION]", get_robot_plan(args=args, step=None, with_obs=False)
        )
        ans, _ = llm_prompter.query(
            prompt=prompt,
            sampling_params=prompt_info["prompt-plan"]["params"],
            save=prompt_info["prompt-plan"]["save"],
            save_dir=save_dir,
        )
        print(prompt["user"], ans)
        print("=====================================")
        ...
        prompt["user"] = prompt_info["prompt-plan-step"]["template-user"].replace(
            "[PREV_PROMPT]", prompt["user"] + " " + ans
        )
        step, _ = llm_prompter.query(
            prompt=prompt,
            sampling_params=prompt_info["prompt-plan-step"]["params"],
            save=prompt_info["prompt-plan-step"]["save"],
            save_dir=save_dir,
        )
    ...
    with open(
        "real_world/state_summary/{}/{}".format(args.folder_name, reasoning_json_name),
        "w",
    ) as f:
        json.dump(reasoning_dict, f)
...
def create_folders(f_name):
    ...
    os.system(
        f"mkdir -p real_world/state_summary/{f_name}/mdetr_obj_det/clip_processed_det/"
    )
...
def config_parser(parser=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="mdetr",
        help="which object detection model to use",
    )
    parser.add_argument(
        "--audio_ver",
        type=int,
        default=1,
        help="1 is with detected audio, 0 is without audio",
    )
    parser.add_argument(
        "--ablation_type", type=int, default=0, help="which experiment to run"
    )
    return parser
...
def get_interact_actions(meta_data, total_frames, args, task_json):
    ...
    for step_idx in range(0, total_frames):
        ...
        if curr_stage != prev_stage:
            ...
            if args.folder_name == "heatPotato2" and prev_stage == 8:  # 7268
                step_idx = step_idx - 2
            if (
                args.folder_name == "heatPotato2" and prev_stage == 9
            ):  # 10000 - 90 = 9910
    ...
    last_idx = (
        len(os.listdir(f"real_world/data/{args.folder_name}/videos/0/0/color")) - 2
    )
...
def run_real_world_pipeline(args, task_info):
    ...
    if not os.path.exists(
        f"real_world/state_summary/{args.folder_name}/{args.obj_det}_obj_det/det/{total_frames}.pickle"
    ):
    ...
    interact_actions_start_idx = [
        idx[0]
        for idx in interact_actions.keys()
        if "Ignore" not in interact_actions[idx]
    ]
    interact_actions_end_idx = [
        idx[1]
        for idx in interact_actions.keys()
        if "Ignore" not in interact_actions[idx]
    ]
    ...
    if args.audio_ver == 1:
        ...
        if os.path.exists(audio_path):
            ...
            detected_sounds = get_sound_events(
                audio_path=audio_path, volume_thresh=volume_thresh
            )
        else:
        ...
        for sound_range in detected_sounds.keys():
            ...
            sound_det_idx_dict[sound_range[1] * 30 + total_ignore_length] = (
                detected_sounds[sound_range]
            )
    ...
    if not os.path.exists(f"real_world/state_summary/{args.folder_name}/global_sg.pkl"):
        ...
        for step_idx in range(0, total_frames):
            if (
                step_idx != 0
                and step_idx not in interact_actions_end_idx
                and step_idx not in sound_det_idx_dict
            ):
                continue
            ...
            print(local_sg)
            with open(
                f"real_world/state_summary/{args.folder_name}/local_graphs/local_sg_{step_idx}.pkl",
                "wb",
            ) as f:
        ...
        with open(
            f"real_world/state_summary/{args.folder_name}/global_sg.pkl", "wb"
        ) as f:
            pickle.dump(global_sg, f)

        # save keyframes to disk
        with open(
            "real_world/state_summary/{}/L1_key_frames.txt".format(args.folder_name),
            "w",
        ) as f:
        ...
        with open(
            f"real_world/state_summary/{args.folder_name}/global_sg.pkl", "rb"
        ) as f:
            global_sg = pickle.load(f)
    ...
    L1_summary_file_name = (
        "state_summary_L1_BLIP2.txt"
        if args.ablation_type == 5
        else "state_summary_L1.txt"
    )
    if not os.path.exists(
        f"real_world/state_summary/{args.folder_name}/{L1_summary_file_name}"
    ):
        # if True:
        ...
        for step_idx in key_frames:
            ...
            if args.ablation_type == 5:
                with open(
                    f"real_world/BLIP2_captions/{args.folder_name}/caption_{step_idx}.txt",
                    "r",
                ) as f:
            ...
            if (
                len(L1_captions) != 0
                and caption.split(".")[0] == L1_captions[-1].split(".")[0]
            ):
                continue
            state_summary_L1 += caption
            L1_captions.append(caption)
        with open(
            f"real_world/state_summary/{args.folder_name}/{L1_summary_file_name}", "w"
        ) as f:
        ...
        L1_captions = []
        with open(
            f"real_world/state_summary/{args.folder_name}/{L1_summary_file_name}", "r"
        ) as f:
            L1_captions = f.readlines()
        state_summary_L1 = "".join(L1_captions)

    L2_summary_file_name = (
        "state_summary_L2_BLIP2.txt"
        if args.ablation_type == 5
        else "state_summary_L2.txt"
    )
    if not os.path.exists(
        f"real_world/state_summary/{args.folder_name}/{L2_summary_file_name}"
    ):
        # if True:
        ...
        with open(
            "real_world/state_summary/{}/{}".format(
                args.folder_name, L2_summary_file_name
            ),
            "w",
        ) as f:
