...
def _make_detr(
    ...
    transformer = Transformer(
        d_model=hidden_dim, return_intermediate_dec=True, text_encoder_type=text_encoder
    )
    detr = MDETR(
        backbone,
        transformer,
...
def mdetr_efficientnetB5_gqa(pretrained=False, return_postprocessor=False):
    ...
    model = _make_detr(
        "timm_tf_efficientnet_b5_ns", qa_dataset="gqa", contrastive_align_loss=False
    )
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://zenodo.org/record/4721981/files/gqa_EB5_checkpoint.pth",
            map_location="cpu",
            check_hash=True,
        )
        model.load_state_dict(checkpoint["model"])
...
def mdetr_resnet101_phrasecut(
    pretrained=False, threshold=0.5, return_postprocessor=False
):
    """
    MDETR R101 with 6 encoder and 6 decoder layers.
...
def mdetr_efficientnetB3_phrasecut(
    pretrained=False, threshold=0.5, return_postprocessor=False
):
    """
    MDETR ENB3 with 6 encoder and 6 decoder layers.
    ...
    """
    model = _make_detr(
        "timm_tf_efficientnet_b3_ns", mask=True, contrastive_align_loss=False
    )
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
        ...
        # Backward-compatibility: older checkpoints may include this buffer key.
        state_dict.pop("detr.transformer.text_encoder.embeddings.position_ids", None)
...
def mdetr_lvis(pretrained=False, return_postprocessor=False):
    """
    MDETR ENB3 with 6 encoder and 6 decoder layers.
    Pretrained on our combined aligned dataset of 1.3 million images paired with text.
    """
